      ...
      <main className="mx-auto w-full max-w-7xl space-y-14 px-6 pb-16 pt-36">
        ...
        <section>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {principles.map((item) => (
              <article key={item.title} className="glass-card rounded-xl p-6">
        ...
        <section className="glass-card rounded-xl p-8">
          <div className="grid grid-cols-2 gap-6 text-center md:grid-cols-4">
            {[
              ['$4.2B+', 'Annual GMV Processed'],
              ['2.5k', 'Active Routes Managed'],
              ['15%', 'Avg Fuel Efficiency Gain'],
              ['24/7', 'Mission-Critical Support'],
            ].map(([value, label]) => (
              <div key={label}>
                <p className="text-3xl font-semibold text-[#c0c1ff]">{value}</p>
